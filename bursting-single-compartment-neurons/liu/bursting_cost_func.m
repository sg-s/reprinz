...
function [C, cost_vector] = bursting_cost_func(x, show_soma, ax)
...
[ons,offs] = computeOnsOffs(Ca > max(Ca)/2);
...
if nargout == 0
	disp(['Level 3 (depol block) cost is ' oval(cost_vector(3))])
end
...
% isi_cost = isi_cost_func(isis);

% if nargout == 0
% 	disp(['Level 3 (depol block) cost is ' oval(isi_cost)])
% end
...
period_cost = level_cost*bin_cost(cycle_period_range,all_periods);
duration_cost = level_cost*bin_cost(burst_duration_range,nanmean(all_durations));
all_duty_cycle = nanmean(all_durations)./all_periods;
dc_cost = level_cost*bin_cost(duty_cycle_range,all_duty_cycle);
...
cost_vector(4) = period_cost + duration_cost + dc_cost;
